...
command "go build hello-world.go && docker build -t teaching/demo/03 ."
command "docker run --rm --name ${CONTAINER_NAME} teaching/demo/03"

printf "\n"

question "What happens if we try to run bash?"
command "docker run --rm -it --name ${CONTAINER_NAME} teaching/demo/03 bash"

printf "\n"

info "We can inspect the image with \`dive\` to see what's inside"
command "dive teaching/demo/03"
