...
command "docker build -t teaching/demo/04 ."
command "docker run --rm --name ${CONTAINER_NAME} teaching/demo/04"

printf "\n"

info "But we can also allow the container to access some files on the host"
command "docker run --rm --name ${CONTAINER_NAME} -v ./config.json:/config.json:ro teaching/demo/04"

printf "\n"

info "And we can also pass environment variables"
command "docker run --rm --name ${CONTAINER_NAME} -e APP_ID=2 teaching/demo/04"
